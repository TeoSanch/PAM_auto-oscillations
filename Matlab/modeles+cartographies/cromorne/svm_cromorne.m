% La fonction svm_cromorne calcule la cartographie des états de la
% cromorne en fonction d'un descripteur donné.
% Les entrées sont :
% - descriptor (string) : 
...
function [] = svm_cromorne(descriptor, f0,...
    ...
    % Parmètres du DOE
    cromorne_samples = [gamma_sample zeta_sample];
    %% Audio descriptor
    ...
    classes_doe = f(cromorne_samples);      % Classes associées au DOE

    %% Training
    sprintf('Train the SVM model')
    % Avec la fonction MATLAB
    % svm = svmtrain(cromorne_sample, classes_init, 'kernel_function', 'polynomial', 'ShowPlot',true);
    % Avec CODES
    svm = CODES.fit.svm(cromorne_samples, classes_doe,'solver','dual');
    %% Adaptive sampling
    ...
    if strcmp(descriptor, 'isRough') ||strcmp(descriptor, 'isBright')
        ...
        title('SVM roughness of the cromorne model');
   ...
   %% Save SVM edsd & images
    ...
    mkdir('./cromorne/matFiles');
    saveMatFilename = strcat('./cromorne/matFiles/',saveMatFilename);
    save(saveMatFilename, 'finalSVM')

    % Save graph as an image
    savePngFilename = strcat(savefilename,'.png');
    mkdir('./cromorne/png');
    savePngFilename = strcat('./cromorne/png/',savePngFilename);
    figure(2)
