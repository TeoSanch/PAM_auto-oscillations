% La fonction svm_basson calcule la cartographie des états de la
% basson en fonction d'un descripteur donné.
% Les entrées sont :
% - descriptor (string) : 
...
function [] = svm_basson(descriptor, f0,...
    ...
    classes_doe = f(basson_samples);      % Classes associées au DOE

    %% Training
    sprintf('Train the SVM model')
    % Avec la fonction MATLAB
    % svm = svmtrain(basson_sample, classes_init, 'kernel_function', 'polynomial', 'ShowPlot',true);
    % Avec CODES
    svm = CODES.fit.svm(basson_samples, classes_doe,'solver','dual');
    %% Adaptive sampling
...
%
   ...
   %% Save SVM edsd & images
    ...
    mkdir('./basson/matFiles');
    saveMatFilename = strcat('./basson/matFiles/',saveMatFilename);
