% La fonction svm_saxophone calcule la cartographie des états de la
% saxophone en fonction d'un descripteur donné.
% Les entrées sont :
% - descriptor (string) : 
...
function [] = svm_saxophone(descriptor, f0,...
    ...
    gamma_min = 0.2; gamma_max = 1.1;       % Pression dans la bouche
    zeta_min = 0; zeta_max = 6;             % Parmètre d'anche
    ...
    % Parmètres du DOE
    saxophone_samples = [gamma_sample zeta_sample];
    %% Audio descriptor
    ...
    classes_doe = f(saxophone_samples);      % Classes associées au DOE

    %% Training
    sprintf('Train the SVM model')
    % Avec la fonction MATLAB
    % svm = svmtrain(saxophone_sample, classes_init, 'kernel_function', 'polynomial', 'ShowPlot',true);
    % Avec CODES
    svm = CODES.fit.svm(saxophone_samples, classes_doe,'solver','dual');
    %% Adaptive sampling
   ...
   %% Save SVM edsd & images
    ...
    mkdir('./saxophone/matFiles');
    saveMatFilename = strcat('./saxophone/matFiles/',saveMatFilename);
    save(saveMatFilename, 'finalSVM')

    % Save graph as an image
    savePngFilename = strcat(savefilename,'.png');
    mkdir('./saxophone/png');
    savePngFilename = strcat('./saxophone/png/',savePngFilename);
    figure(2)
