...
freq = hslider("Frequency", 220, 50, 1300, 1);
...
process = pext
    letrec {
	'x = (b1a * p(u(v,w(x)),v) + a1a * x + a2a * x') * play;
	'v = (-a1 * u(v,w(x)) - b0 * (u(v,w(x))@(D-1)) + a1 * p(u(v,w(x)),v) - b0 * (p(u(v,w(x)),v)@(D-1))) * play;
	'pext = p(u(v,w(x)),v) - p(u(v',w(x')),v');
    };



p(u,v) = (bc0 * u + v) * play;
u(v,w) = (0.5 * ma.signum(gamma - v) * (-bc0 * w^2 + w * sqrt((bc0*w)^2 + 4*abs(gamma-v)))) * play;
w(x)   = (heaviside(1 - gamma + x) * zeta * (1 - gamma + x)) * play;
